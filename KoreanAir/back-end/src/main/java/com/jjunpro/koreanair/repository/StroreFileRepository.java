...
@Repository
public interface StroreFileRepository extends JpaRepository<StoreFile, String>{
	long countByFileBoNum(long num);
	
	// 게시판 미리보기 이미지 업데이트 쿼리
	StoreFile findTop1ByFileBoNumAndFileDivisionOrderByFileNoAsc(long num, int division);
	
	// 게시판 모든 파일 가져오는 쿼리
	StoreFile[] findByFileBoNumOrderByFileNoAsc(long num);
	
	// 게시글의 이미지 파일만 가져오는 쿼리
	StoreFile[] findByFileBoNumAndFileDivisionOrderByFileNoAsc(long num, int division);
	
	// 게시글 수정시 파일 삭제가 전송된경우 bo_num 값을 확인하여 파일 삭제(숨김) 처리
	StoreFile findByFileBoNumAndId(long num, String file);
}
