...
public class RepositoryUtill
{
    ...
    public OrderSpecifier getSearchOrderBy(SearchDTO searchDTO)
    {
        ...
        {
            case "like" :
                return orderByData = new OrderSpecifier(Order.DESC, qUniversity.uniLike.size());

            case "comment" :
                return orderByData = new OrderSpecifier(Order.DESC, qUniversity.comments.size());

            default:
                return orderByData = new OrderSpecifier(Order.DESC, qUniversity.modifiedDate);
        }
    }
}
